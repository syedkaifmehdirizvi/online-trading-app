...
public class OrderController 
{
    ...
    // remove this as we will have different pages for buy and sell orders
    ...
    public ModelAndView showReplaceOrderForm(@PathVariable("orderId") Integer orderId) {
        Order order = orderService.getOrderById(orderId).orElseThrow(() -> new RuntimeException("Order not found"));
        // Add necessary data to the model
        List<User> users = userService.getAllUsers();
        List<Instrument> instruments = instrumentService.getAllInstruments();
        ModelAndView mav = new ModelAndView("replaceOrder");
        mav.addObject("users", users);
        mav.addObject("instruments", instruments);
    ...
    public ModelAndView showFilledOrders() {
        List<Order> orders = orderService.getFilledOrders();
        // Add necessary data to the model
        ModelAndView mav = new ModelAndView("filledOrders");
